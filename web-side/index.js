$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("http://RKG_inventory/invClose");
		}
	};
});
...
const updateDrag = () => {
	...
	$('.use').droppable({
		...
		drop: function(event,ui){
			...
			$.post("http://RKG_inventory/useItem", JSON.stringify({
				item: itemData.key,
				type: itemData.type,
	...
	$('.drop').droppable({
		...
		drop: function(event,ui){
			...
			$.post("http://RKG_inventory/dropItem", JSON.stringify({
				item: itemData.key,
				amount: Number($("#amount").val())
			}))
	...
	$('.send').droppable({
		...
		drop: function(event,ui){
			...
			$.post("http://RKG_inventory/sendItem", JSON.stringify({
				item: itemData.key,
				amount: Number($("#amount").val())
			}))
...
const updateMochila = () => {
	document.getElementById("amount").value = "";
	$.post("http://RKG_inventory/requestMochila",JSON.stringify({}),(data) => {
