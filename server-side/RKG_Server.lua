...
vRPN = {}
Tunnel.bindInterface("RKG_inventory",vRPN)
Proxy.addInterface("RKG_inventory",vRPN)
			...
			if parseInt(amount) > 0 then
				if vRP.getInventoryWeight(nuser_id) + vRP.getItemWeight(itemName) * amount <= vRP.getInventoryMaxWeight(nuser_id) then
					if vRP.tryGetInventoryItem(user_id,itemName,amount) then
						...
						TriggerClientEvent('RKG_inventory:Update',source,'updateMochila')
						TriggerClientEvent('RKG_inventory:Update',nplayer,'updateMochila')
						return true
					end
				end
			else
...
function vRPN.useItem(itemName,type,ramount)
	...
	if user_id and ramount ~= nil and parseInt(ramount) >= 0 and not actived[user_id] and actived[user_id] == nil then
		if type == "usar" then
			...
			elseif itemName == "morfina" then -- RKG_inventory:Update
				local paramedico = vRP.getUsersByPermission("polpar.perm")
						...
						repeat
							if active[parseInt(user_id)] == 0 then
								...
								if vRP.tryGetInventoryItem(user_id,itemName,1) then
									TriggerClientEvent('syncpneu',source)
